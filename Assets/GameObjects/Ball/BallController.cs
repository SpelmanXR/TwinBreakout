...
public class BallController : MonoBehaviour
{
    ...
    static int Score = 0;      //stores the score
    static int numBlocksHit = 0;   //stores the # of blocks hit so far.  The game ends when this number
    static int OldNumBlocksHit = 0;
    ...
    static bool bGameOver = false;     //flag set to true when the last block is hit.
    ...
    void Start()
    {
        BallAngle = (45f + Random.Range(-15f, 15f)) * Mathf.Deg2Rad * (2 * Random.Range(0, 2) - 1);
        //note that (2 * Random.Range(0, 2) - 1) evaluates to -1 or +1
        ...
        InvokeRepeating("CalculateHitRate", 1.0f, 1.0f);
    }
    ...
    {
        int BlocksPerSecond = numBlocksHit - OldNumBlocksHit;
        if (BlocksPerSecond > 3) backgroundMaskController.SetMask(BackgroundMaskController.MaskType.AweMask);
        OldNumBlocksHit = numBlocksHit;
    }
